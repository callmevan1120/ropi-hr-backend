...
export class AttendanceService {
  ...
  async createCheckin(data: any) {
    ...
    try {
      ...
      // Backend sekarang PASTI MENGGUNAKAN Shift yang dikirim Frontend 
      // (Karena Frontend yang mengalkulasi logic Kantor/Satpam, atau Active Shift Outlet)
      const payload: any = {
        ...
        custom_verification_image: data.custom_verification_image, // Untuk Outlet Kiri
  ...
  async proxyFile(filePath: string): Promise<{ buffer: Buffer; contentType: string }> {
    ...
    const response = await firstValueFrom(
      this.httpService.get(url, { headers: { Authorization: authHeader }, responseType: 'arraybuffer' }),
    );
    ...
    try {
      ...
      const fileRes = await firstValueFrom(
        this.httpService.get(`${erpUrl}/api/resource/File`, {
          headers: { Authorization: authHeader },
          params: {
            filters: JSON.stringify([['attached_to_doctype', '=',  'Leave Application'], ['attached_to_name',    'in', docNames]]),
      ...
      const result = leaveList.map((leave) => ({ ...leave, attachment: attachmentMap[leave.name] ?? null }));
    ...
    try {
      ...
      if (docName && data.attachment) {
        try {
          ...
          const beforeFile = [`--${boundary}`, `Content-Disposition: form-data; name="file"; filename="${safeFileName}"`, `Content-Type: ${mimeType}`, '', ''].join('\r\n');
          const afterFile = ['', `--${boundary}`, `Content-Disposition: form-data; name="is_private"`, '', '0', `--${boundary}`, `Content-Disposition: form-data; name="doctype"`, '', 'Leave Application', `--${boundary}`, `Content-Disposition: form-data; name="docname"`, '', docName, `--${boundary}--`, ''].join('\r\n');

          const bodyBuffer = Buffer.concat([Buffer.from(beforeFile, 'utf-8'), fileBuffer, Buffer.from(afterFile, 'utf-8')]);

          await firstValueFrom(
            this.httpService.post(`${erpUrl}/api/method/upload_file`, bodyBuffer, {
              headers: { Authorization:   authHeader, 'Content-Type':  `multipart/form-data; boundary=${boundary}`, 'Content-Length': bodyBuffer.length.toString() },
            }),
          );
        } catch (e: any) { console.error('Upload Gagal:', e.message); }
      }
